...
namespace Microsoft.Azure.EngagementFabric.ResourceProviderWebService
{
    ...
    internal sealed class ServiceEventSource : EventSource
    {
        ...
        [NonEvent]
        public void ServiceException(StatelessServiceContext serviceContext, string requestId, Exception ex)
        {
            if (this.IsEnabled())
            {
                ServiceError(
        ...
        private const int ServiceErrorEventId = 7;
        [Event(ServiceErrorEventId, Level = EventLevel.Error, Message = "[{7}] {8}")]
        public void ServiceError(
            ...
            string message)
        {
            WriteEvent(ServiceErrorEventId, serviceName, serviceTypeName, replicaOrInstanceId, partitionId, applicationName, applicationTypeName, nodeName, requestId, message);
        }

        [NonEvent]
        public void ServiceRequestTrace(StatelessServiceContext serviceContext, string requestId, string message)
        {
            if (this.IsEnabled())
            {
                ServiceRequestTrace(
        ...
        private const int ServiceRequestTraceEventId = 8;
        [Event(ServiceRequestTraceEventId, Level = EventLevel.Informational, Message = "[{7}] {8}")]
        public void ServiceRequestTrace(
            string serviceName,
            string serviceTypeName,
            long replicaOrInstanceId,
            Guid partitionId,
            ...
            string message)
        {
            WriteEvent(ServiceRequestTraceEventId, serviceName, serviceTypeName, replicaOrInstanceId, partitionId, applicationName, applicationTypeName, nodeName, requestId, message);
        }
        #endregion
